...
- VERSION 0.1.0 [1/9]
  - [ ] FEATURE: implement scrollbar-overflow for large viewport ::
  - [ ] FEATURE: Mobile UX ::
    - [ ] use viewport detection stuff to fork between two different layout arrangements
      - [ ] mobile: three rows, top row is just search results, 2nd row is graph, 3rd row is left, then right details
      - [ ] desktop: existing setup
    - [ ] graph has 100% width and height of viewport - 50px or something..
