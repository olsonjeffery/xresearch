...
- VERSION 0.1.0
  - [-] REFACTOR: switch CSS grid/styling to bootstrap ::
    - [ ] rework node list display to use tables, instead of ul, and share more formatting code
      - [ ] preserve click behavior when aprop
      - [ ] use slate theme colors instead of default red, green, blue & gray
    - [ ] put cost, points, etc into a new left-nav summary box
    - [ ] graph work
      - [ ] put border on graph component w/ drop shadow
      - [ ] make background a darker gray
      - [ ] integrate slate theme colors into graph component for node/edge colors
