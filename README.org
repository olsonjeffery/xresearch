...
- PENDING [0/11]
  - [ ] REFACTOR: switch CSS grid/styling to bootstrap
    - [ ] push most of the summary-row stuff into a page nav w/ title & search bar
    - [ ] expand graph display to entire width of page in the center
    - [ ] place left/right columns beneath graph, make wider (col-6 or whatever)
...
- VERSION 0.0.3 [3/8]
  - [ ] FEATURE: add manufacturing tasks & item data
    - [ ] annotate different topic types as they appear in researchData: item, idea, manufacture
    - [ ] add researchData entries for manufacturables when encountered via topic
    - [ ] item annotation for existing researachable items
      - [ ] add their requiresBuy/whatever as a dependency and map the inverse
    - [ ] add a requiresBuy display for nodes that have them in Left node list or wahtever
    - [ ] add items not in research tree (eg mortar)
      - [ ] index addedTopics against unique keys in .items to get list of items that needed to be added
    - [ ] show manufacturing requirements on right node list for manufacturables
    - from Dioxine:
  ...
  - [ ] FEATURE: show STR_ id in node details
  ...
  - [ ] BUG: search bar magnifying glass appears in wrong place in firefox
